...
/**
 * This class checks the conditions based on attribute value accepts AND, OR and
 * Bracket attributes Fieldname should be in bundle.property format Currently
 * supports only with Simple String and Enumerated String datatype
 * 
 * @author IBM, Inc.
 * 
 */
public class BundlePropertyConditionalUtil {
	...
	private static final Logger logger = LoggerUtilExtended.getLogger(BundlePropertyConditionalUtil.class);

	private static final String SEPARATOR_COLON = ":";

	// BundlePropertyConditionalUtil extended to allow parent type names in bundle
	// field names
	...
	/**
	 * validating the input condition for equal number of open and close brackets
	 * 
	...
	/**
	 * Can be overridden to manipulate the resource and condition checking whether
	 * the condition has any brackets or operators if true - split it
	 */
	protected boolean processConditionAgainstResource(String input, IGRCObject object) {
		boolean conditionValue = false;
		String delimiter = EQUAL_OPERATOR;
		logger.debug("input condition is : " + input);
		try {
			if (input.contains(NOT_EQUAL_OPERATOR)) {
				delimiter = NOT_EQUAL_OPERATOR;
			}
			String[] fieldArray = input.split(delimiter);
			String fieldValue = getPropertyValueAsString(fieldArray[0], object);
			logger.debug("fieldValue from resource is : " + fieldValue);
			String formatInputValue = fieldArray[1];
			formatInputValue = formatInputValue.replaceAll("\"", "").trim();
			formatInputValue = formatInputValue.replaceAll(ESCAPE_CHARACTER + ESCAPE_CHARACTER, "").trim();
			if (EQUAL_OPERATOR.equalsIgnoreCase(delimiter)) {
				if ("null".equalsIgnoreCase(formatInputValue.trim())) {
					if (fieldValue == null) {
						conditionValue = true;
					}
				}
				else if (fieldValue != null && fieldValue.equalsIgnoreCase(formatInputValue)) {
					conditionValue = true;
				}
			}
			else if (NOT_EQUAL_OPERATOR.equalsIgnoreCase(delimiter)) {
				if ("null".equalsIgnoreCase(formatInputValue.trim())) {
					if (fieldValue != null) {
						conditionValue = true;
					}
			...
			// Suppressing the error here : This is valid for NULL cases
			logger.error("Error processing the condition against resource " + ex);
			// TODO : how to display error on screen ?
		}
		return conditionValue;
	}
	...
	private String getNextGeneralValidCondition(String tempCondition) {
		...
		if (isAnyCloseBracketOperator(tempCondition)) {
			len = tempCondition.length();
			// int closeBracketIndex = tempCondition.lastIndexOf(CLOSE_BRACKET, len - 1);
	 ...
	 */
	protected String getPropertyValueAsString(String fieldName, IGRCObject object) throws Exception {
		// Set these up as default values (ass-u-me that the fieldName is of form B:F)
		// the resource,searchFieldName may be amended (if fieldName is of form P:B:F)
		...
		if (StringUtil.isGood(fieldName)) {
			...
			// 2) P:B:F => Field F in Bundle B in the primary parent (of type P) of the
			// object
			...
			if (subNames.length == 3) {
				...
				if (parent != null) {
					...
					// Oops! parent was not set up by the caller of this class AND it was needed
					logger.debug("FieldName '" + fieldName
							+ "' requires a parent IGRCObject to be present! Remember to use the setParent method on this class!");
					resource = null;
				}
			}
			// At this point resource,searchFieldName could have been altered (and resource
			// is possibly null)
			// At this point we have 3 possibilities
			// 1) original object + field name are unchanged - but still could be an invalid
			// combination
			// 2) object, field name both changed to parent+field name - still possibly
			// invalid combination
			// 3) object unchanged but field name is P:B:F (=> parent not found/wrong type)
			...
			if (propertyType != null) {
				...
				if (DataType.CURRENCY_TYPE.equals(propertyType)) {
					...
					if (enumField != null) {
						// logger.debug("Got the ENUM field-READ? '"+enumField.canRead()+"' WRITE?
						// '"+enumField.canWrite()+"'");
				...
				else if (DataType.INTEGER_TYPE.equals(propertyType)) {
					logger.debug("It's a INTEGER");
					IIntegerField intField = (IIntegerField) resource.getField(sourceFieldName);
					if (intField != null)
						srcValue = intField.getValue().toString();
				}
				else {
	...
	private boolean isAnyOpenBracketOperator(String value) {
		 ...
		 * !value.contains(ESCAPE_CHARACTER+OPEN_BRACKET)){ conditionValue = true; }
		 * return conditionValue;
		 */
	...
	/**
	 * checking whether the condition has any brackets or operators return boolean
	 */

...
import static com.ibm.openpages.ext.citi.workflow.constant.NewDSMTLinkBaseIDRemovalConstants.*;
 ...
 * a) get all DSMT Links (Citi_DSMT_Link) under AE (AuditableEntity) where DSMT Link Status (Citi-DL:Status) = Pending Removal – these will be the DSMTs that the approval is for to be removed
 * b) get the based ids of all of these DSMT Links Citi_DSMT_Link:Citi-DL:BaseID (e.g. if there were 3 DSMT Links where status = pending removal, then base ids would be 1234, 6544, 7574)
 ...
 * e) Update all the Audits from the list above:
 * 		AuditProgram:Citi-Aud:DSMTList== Remove the DSMT ID's of the remove DSMT's --> list from b above. Note: This should be remove from the existing list.
 * </P>
 *
 * @custom.feature : Workflow Custom Actions
 * @custom.category : Custom Actions
 */
public class NewDSMTLinkBaseIDRemovalCustomAction extends BaseWorkflowCustomAction {
     ...
     * @param properties
     */
    public NewDSMTLinkBaseIDRemovalCustomAction(IWFOperationContext context, List<IWFCustomProperty> properties) {
    ...
    protected void process() throws Exception {
        ...
        try {
            ...
            logger.debug("\nStarting NewDSMTLinkBaseIDRemovalCustomAction...");
            ...
            for (String auditId : auditSet) {
                ...
                logger.info("Before removal current Audit DSMT Link List: " + currentAuditDSMTLinkList);
                currentAuditDSMTLinkList.removeAll(dsmtLinkSet);
                logger.info("After removal current Audit DSMT Link List: " + currentAuditDSMTLinkList);

                if (isListNotNullOrEmpty(currentAuditDSMTLinkList)) {
                    ...
                    logger.info("New DSMT Link list is empy or null : " + currentAuditDSMTLinkList);
                    fieldUtil.setFieldValue(auditObj, AUDIT_NEW_DSMT_LINK_LIST_FIELD, EMPTY_STRING);
            ...
            logger.error(
                    "EXCEPTION!!!!!!!!!!! An exception occurred while running the NewDSMTLinkBaseIDRemovalCustomAction : " + getStackTrace(ex));
        }

...
public class AuditIssueHandler extends BaseEventHandler {
    public static final String MESSAGE = "Push Audit Issue to iCAPS Trigger ERROR";
    private Logger logger = LogManager.getLogger(AuditIssueHandler.class);
    ...
    public boolean handleEvent(CreateResourceEvent event) {
        init(event);
        logger.debug("*** Start Audit Issue Create Handler ***");
        ...
        logger.debug("*** End Audit Issue Create Handler ***");
        return result;
    }
    ...
    public boolean handleEvent(UpdateResourceEvent event) {
        init(event);
        logger.debug("*** Start Audit Issue Update Handler ***");
        boolean result = executePush((IGRCObject) event.getUpdatedResource(), event.getContext());
        logger.info("Total transaction time = " + (System.currentTimeMillis() - startTime) + " milliseconds");
        logger.debug("*** End Audit Issue Update Handler ***");
        return result;
    }
    ...
    public boolean executePush(IGRCObject object, Context context) {
        ...
        try {
            logger.debug("will push to iCAPS a grcObject with reference: " + object + " Name : "+ object.getName() + " Id : " + object.getId());
            icapsService.pushAuditIssue(object);
            result = true;
        } catch (Exception e) {

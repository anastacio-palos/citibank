...
public class CAPChangeCounterHandler extends BaseEventHandler {
    public static final String MESSAGE = "Push CAP Issue to iCAPS Trigger ERROR";
    private Logger logger = LogManager.getLogger(CAPChangeCounterHandler.class);
    ...
    public CAPChangeCounterHandler() {
        super();
    }
    ...
    public boolean handleEvent(CreateResourceEvent event) {
        init(event);
        logger.debug("*** Start CAP Counter Create Handler  ***");
        ...
        logger.debug("*** End CAP Counter Create Handler ***");
        return result;
    }
    ...
    public boolean handleEvent(UpdateResourceEvent event) {
        init(event);
        logger.debug("*** Start CAP Counter Update Handler ***");
        boolean result = executePush((IGRCObject) event.getUpdatedResource(), event.getContext());
        logger.info("Total transaction time = " + (System.currentTimeMillis() - startTime) + " milliseconds");
        logger.debug("*** End CAP Counter Update Handler ***");
        return result;
    }
    ...
    public boolean executePush(IGRCObject object, Context context) {
        ...
        try {
            logger.debug("will change counter on a grcObject with reference: " + object + " Name : "+ object.getName() + " Id : " + object.getId());
            icapsService.incrementCAPChangeCounter(object);
            result = true;
        } catch (Exception e) {

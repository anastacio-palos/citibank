...
import static com.ibm.openpages.ext.ui.constant.AuditIERAMRConstants.*;
...
@Service("auditIerAmrServiceImpl")
public class AuditIERAMRServiceImpl extends AuditMappingResourceBaseServiceImpl
    ...
    public void initServiceImpl() {
        ...
        logger = loggerUtil.getExtLogger(AMR_IER_LOG_FILE_NAME);
    }
    ...
    @Override
    public DSMTLinkHelperAppInfo addDSMTLinksToAMR(DSMTLinkHelperAppInfo dsmtLinkHelperAppInfo) throws Exception {
        return null;
    }
     ...
     * </p>
     * <p>
     *     Ancestor Audit Query : This query will return all the ancestor audit's associated to the current AMR
    ...
    public DataGridInfo getAncestorIssues(DSMTLinkHelperAppInfo dsmtLinkHelperInfo) throws Exception {
        ...
        logger.info("AMR IER getAncestorIssues() Start");
        ...
        String ancestorAuditAssociatedToCurrentAMRQuery = GET_ALL_ANCESTOR_AUDIT_ASSOCIATED_TO_CURRENT_AMR_QUERY.replaceAll(RESOURCE_ID, amrResourceId);
        ...
        logger.info("Ancestor BMQS associated to current AMR query: " + ancestorAuditAssociatedToCurrentAMRQuery);
        logger.info("Issues directly associated to AMR query: " + issuesDirectlyAssociatedToAMRQuery);

        // Run Queries
        List<String> ancestorBMQSAssociatedToCurrentAMRList = grcObjectSearchUtil.getMultipleValuesFromQueryAsList(ancestorAuditAssociatedToCurrentAMRQuery, false);
        ...
        logger.info("AMR IER getAncestorIssues() End");
    ...
    public DSMTLinkHelperAppInfo processScopeIssuesFromAMR(DSMTLinkHelperAppInfo dsmtLinkHelperAppInfo) throws Exception {
        ...
        logger.info("AMR IER processScopeIssuesFromAMR() START");
		...
		*/
        ...
        logger.info("AMR IER processScopeIssuesFromAMR() END");
    ...
    private List<AncestorIssueInfo> processAncestorIssuesList(List<String> ancestorBMQSAssociatedToCurrentAMRList, List<String> issuesDirectlyAssociatedToAMRList) throws Exception {
        ...
        logger.info("AMR IER processAncestorIssuesList() START");
        ...
        if(isListNotNullOrEmpty(ancestorBMQSAssociatedToCurrentAMRList))
            for(String ancestorAuditResourceId : ancestorBMQSAssociatedToCurrentAMRList) {

                // Run Query
                logger.info("Get all issues associated to each ancestor BMQS query: " + GET_ALL_ISSUES_ASSOCIATED_TO_EACH_ANCESTOR_AUDIT_QUERY.replaceAll(RESOURCE_ID, ancestorAuditResourceId));
                ITabularResultSet resultSet = grcObjectSearchUtil.executeQuery(GET_ALL_ISSUES_ASSOCIATED_TO_EACH_ANCESTOR_AUDIT_QUERY.replaceAll(RESOURCE_ID, ancestorAuditResourceId), true);
        ...
        logger.info("AMR IER processAncestorIssuesList() END");
    ...
    private DSMTLinkHelperAppInfo associateIssues(DSMTLinkHelperAppInfo dsmtLinkHelperAppInfo, List<String> IssuesToBeAssociatedList) throws Exception {
        ...
        logger.info("AMR IER associateIssues() START");
        ...
        if (isListNotNullOrEmpty(IssuesToBeAssociatedList)) {
			...
			/*
                Iterate through the IssuesToBeAssociatedList
        ...
        logger.info("AMR IER associateIssues() END");
    ...
    private DSMTLinkHelperAppInfo dissociateIssues(DSMTLinkHelperAppInfo dsmtLinkHelperAppInfo, List<String> IssuesToBeDissociatedList) throws Exception {
        ...
        logger.info("AMR IER dissociateIssues() START");

        // Local Variables
        AuditMappingReportInfo auditMappingReportInfo = dsmtLinkHelperAppInfo.getAuditMappingReportInfo();
        auditMappingReportInfo = isObjectNotNull(auditMappingReportInfo) ? auditMappingReportInfo : new AuditMappingReportInfo();
        List<Id> issuesAndAEToBeAssociatedList = new ArrayList<Id>();
        List<String> remainingIssuesAssociatedToAMR = new ArrayList<>();
        List<Id> dsmtsToBeUpdatedList = new ArrayList<>();
        ...
        /*
            ...
            If it is not then add that issue resourceID to remainingIssuesAssociatedToAMR.
            </p>
         */
        String issuesDirectlyAssociatedToAMRQuery = GET_ALL_ISSUES_DIRECTLY_ASSOCIATED_TO_CURRENT_AMR_QUERY.replaceAll(RESOURCE_ID, dsmtLinkHelperAppInfo.getObjectID());
        logger.info("Issues directly associated to AMR query : " + issuesDirectlyAssociatedToAMRQuery);
        ...
        for (String eachIssue : issuesDirectlyAssociatedToAMRList)
            if (!IssuesToBeDissociatedList.contains(eachIssue))
                remainingIssuesAssociatedToAMR.add(eachIssue);

        logger.info("List of remaining issues (resource ID's) associated to AMR: " + remainingIssuesAssociatedToAMR);

        /*
            <p>
            Iterate through IssuesToBeDissociatedList, for each resourceID of the issue sin the list
        ...
        // Local Variables
        Map<String, List<String>> dsmtAssociatedToEachIssueMap = null;
        ...
        for (String issueResourceID : IssuesToBeDissociatedList) {
            ...
            logger.info("dmtAssociatedToEachIssueQuery: " + dmtAssociatedToEachIssueQuery);
            dsmtAssociatedToEachIssueMap = grcObjectSearchUtil.getMultipleValuesFromQueryAsMap(dmtAssociatedToEachIssueQuery, false);
        }
        logger.info("Map with DSMT'S info Associated to each Issue" + dsmtAssociatedToEachIssueMap);
        ...
        /*
            ...
            Iterate through the remainingIssuesAssociatedToAMR list, for each issue resourceID in it,
        ...
        if (isMapNotNullOrEmpty(dsmtAssociatedToEachIssueMap)) {
            for (String baseIDKey : dsmtAssociatedToEachIssueMap.keySet()) {
                ...
                for (String eachRemainingIssue : remainingIssuesAssociatedToAMR) {

                    dsmtSharedByMultipleIssuesQuery = dsmtSharedByMultipleIssuesQuery.replaceAll(RESOURCE_ID, eachRemainingIssue);
                    logger.info("Get Issues Associated To DSMT Query : " + dsmtSharedByMultipleIssuesQuery);
                    List<String> issuesAssociatedToDSMTList = grcObjectSearchUtil.getMultipleValuesFromQueryAsList(dsmtSharedByMultipleIssuesQuery, false);
                    logger.info("List of Issues Associated To DSMT: " + issuesAssociatedToDSMTList);

                    if (issuesAssociatedToDSMTList.size() > 0) {
                        isDSMTFound = true;
                        break;
                    }
                }
		...
		*/
        ...
        logger.info("AMR IER dissociateIssues() END");

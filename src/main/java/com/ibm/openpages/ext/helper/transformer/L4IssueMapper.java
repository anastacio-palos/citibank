...
public class L4IssueMapper implements Function<ITabularResultSet, Object> {

    private Log log;
    private IFieldUtil fieldUtil;

    public L4IssueMapper(IFieldUtil fieldUtil, ILoggerUtil iLoggerUtil) {
    ...
    public Object apply(final ITabularResultSet iResultSetRows) {
        ...
        return StreamSupport.stream(resultSet, false).map(v -> {
            ...
            try {
                ...
                opObject.setAeObjRefID(fieldUtil.getFieldValueAsString(v.getField(3)));
                opObject.setScope(fieldUtil.getFieldValueAsString(v.getField(4)));

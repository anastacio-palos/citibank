...
public class AuditProcessAEMapper implements Function<ITabularResultSet, Object> {

    private Log log;
    private IFieldUtil fieldUtil;

    public AuditProcessAEMapper(IFieldUtil fieldUtil, ILoggerUtil iLoggerUtil) {
    ...
    public Object apply(final ITabularResultSet iResultSetRows) {
        ...
        return StreamSupport.stream(resultSet, false).map(v -> {
            ...
            try {
                ...
                opObject.setAeID(fieldUtil.getFieldValueAsString(v.getField(3)));
            }
            catch (Exception ex) {
